...
import parkAdminSchema from '../models/parking/parking.admin.model.js';
...
const adminProtectRoute = async (req,res,next) =>{
    try{
        ...
        if(decoded.role!='Admin'&&decoded.role!='parkAdmin'){
            return res.status(401).json({ Unauthorized: "it is an unauthorized access" });
        }
        const admin =await parkAdminSchema.findById(decoded.userId);
        if(!admin){
            return res.status(401).json({ Unauthorized: "it is an unauthorized access" });
        }
